...
class PairingService:
    ...
    def get_wine_me_pairings(
        self, user_id: int, food: str, friend_ids: Optional[List[int]] = None
    ) -> List[Pairing]:
        user_ids = [user_id]
        if friend_ids:
            user_ids.extend(friend_ids)
        return self.pairing_repository.search_by_food_for_users(user_ids, food, skip=0, limit=None)
